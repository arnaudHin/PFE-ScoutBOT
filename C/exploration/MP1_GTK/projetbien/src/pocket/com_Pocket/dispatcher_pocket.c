...
/**
 *  \fn static void dispatcher_pocket_dispatch(Network_msg network_msg)
 *
...
static void dispatcher_pocket_dispatch(Network_msg network_msg)
{
	TRACE("CMD = %d\r\n",network_msg.cmd);
	switch(network_msg.cmd)
	{
		...
		case(CLOSE):
			...
			break;
		// case(REINIT_POS):
		// 	TRACE("NETWORK MSG ORDER : reinit pos  \r\n");
		// 	pilot_signal_restart_odometry();
		// 	break;
		// case(POSITION_ROBOT_DEST):
		// 	TRACE("NETWORK MSG ORDER : ask robot go \r\n");
		// 	pilot_ask_destination(network_msg.coord);
		// 	break;
		// case(INDICATE):
		// 	TRACE("NETWORK MSG ORDER : indicate connection \r\n");
		// 	network_pilot_indicate_connection_pilot();
		// 	break;
		// case(START_ODOMETRY):
		// 	TRACE("NETWORK MSG ORDER : start odometry \r\n");
		// 	geographer_start_odometry();
		// 	break;
		// case(START_OBSTACLE_SENDER):
		// 	TRACE("NETWORK MSG ORDER : start obstacle sender \r\n");
		// 	obstacle_sender_start_obstacle();
		// 	break;
		// case(STOP_ODOMETRY):
		// 	TRACE("NETWORK MSG ORDER : stop odometry \r\n");
		// 	geographer_stop_odometry();
		// 	break;
		// case(STOP_OBSTACLE_SENDER):
		// 	TRACE("NETWORK MSG ORDER : stop obstacle sender \r\n");
		// 	obstacle_sender_stop_obstacle();
		// 	break;
		case(EMERGENCY_STOP):
			TRACE("NETWORK MSG ORDER : emergency stop \r\n");
			//pilot_signal_emergency_stop();
			break;
...
/**
 *  \fn void dispatcher_pocket_stop()
 *
...
static void dispatcher_decod(unsigned char * buffer, Network_msg * network_msg)
{
	...
	if (network_msg->size > 0)
	{
		...
		postman_pocket_receive_msg(buffer_data,network_msg->size);
...
static void * dispatcher_pocket_run()
{
	...
	do
	{
		...
		postman_pocket_receive_msg(buffer,size_msg);
		dispatcher_decod(buffer,&network_msg);
		dispatcher_pocket_dispatch(network_msg);
	}while(network_msg.cmd!=CLOSE);
	return 0;
}

/**
 *  \fn static void new_dispatcher_pocket_run()
 *
 *  \brief Function dedicated to constantly read test messages
 *             Once the message is received, it decodes it and requests a dispatch
 */
static void * new_dispatcher_pocket_run()
{
	Network_msg network_msg = {.cmd = 30, .size=0}; //, .coord ={0,0}}; //CLOSE
	int size_msg =SIZE_MSG_CMD+SIZE_MSG_SIZE;
	unsigned char buffer[size_msg];
...
/**
 *  \fn static void dispatcher_pocket_start()
 *
 *  \brief Function dedicated to start the dispatcher and postman and pilot.
 */
extern void dispatcher_pocket_start()
{
        postman_pocket_start();
        pilot_start();
        int returnError = pthread_create(&pthread,NULL,&dispatcher_pocket_run,NULL);
        if (returnError == -1)
        {
...
extern void new_dispatcher_pocket_start()
{
        postman_pocket_start();
        pilot_start();
        int returnError = pthread_create(&pthread,NULL,&new_dispatcher_pocket_run,NULL);
        if (returnError == -1)
        {
                perror("Error : creation of thread incorrect in remoteUI start");
                exit(EXIT_FAILURE);
